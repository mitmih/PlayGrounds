...
* `server private key` - закрытый, то есть **секретный** ключ сервера

* `client public key` - открытый ключ клиента

* `client private key` - закрытый, то есть **секретный** ключ клиента

> Зачем нужны ключи и почему их всегда пара?
> 
> Сначала генерируется закрытый ключ, а затем к нему подбирается его открытая часть, так, что теперь они могут работать напарниками:
> 
...
> 
> 
> Возможно, станет понятнее на других примерах:
> 
> * Несложное и подробное объяснение, включая матетматику: ["Асимметричное шифрование. Как это работает?"](https://intsystem.org/security/asymmetric-encryption-how-it-work/)
> 
> * Буквально несколько абзацев и без математики: ["Открытый и закрытый ключ шифрования"](https://firstssl.ru/faq/general-questions/kluch-shifrovania)
