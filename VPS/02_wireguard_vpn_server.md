...
Дело вот в чём: чтобы выдать качественную, в криптографическом смысле, непредсказуемую последовательность случайных чисел, [генератор должен накопить достаточно шума (энтропии)](https://ivirt-it.ru/linux-kernel-rng/), из небольшого набора источников (активность диска, клавиатуры, движения мышки и проч.), которых ещё меньше в случае vps-сервера.
...
```properties
[Interface]
PrivateKey = ODsQHUsi/uh5OoDWo3B2wzPYsUbYD2f67LFI6ejKB2w=
ListenPort = 5140
Address = 10.0.0.4/24
DNS = 8.8.8.8, 1.1.1.1
...
PublicKey = TP9ahELWd2gUzhSZ4Mu5eYtzDgmnF7tfPTfSEPzO0j8=
```
