...
Если совсем коротко, то технология `Shadowsocks` позволяет замаскировать ваш трафик под `https`.

Подробнее об отличиях `Shadowsocks` от `VPN` можно посмотреть, например, здесь:
...
Запускать собственный Outline-сервис будем на виртуальном сервере. Выбор облачного провайдера остаётся за вами. Для семейного сценария использования - пара ноутбуков и смартфонов - подойдёт минимальная конфигурация [Lightsail](https://lightsail.aws.amazon.com). Первый месяц - бесплатно, далее $3.5/месяц.

Будем считать, что вы уже прошли процесс регистрации, привязали банковскую карту, для её будет списан $1 - обещают вернуть :). В течении 24 часов, в случае успешной проверки аккаунта, откроется доступ к запуску `Lightsail`.
...
Рекомендуется сначала обновить списки источников пакетов и сами пакеты:
