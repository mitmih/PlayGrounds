...
С течением времени выданный IP адрес может измениться, например, после перезагрузки сервера. Чтобы этого не произошло и у сервера всегда был один и тот же адрес, нужно привязать к нему статический IP-адрес: `Networking`, `Create Static IP`.
